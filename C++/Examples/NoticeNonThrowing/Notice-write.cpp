...
cbe::Container Notice::createMyContainer(cbe::Container parentContainer,
  ...
  theContainer = *parentContainer.createContainer(name, createContainerError);
  // Check if error
...
void Notice::loadContainerContents(cbe::Container container) {
  ...
  // Call query function with filter
  this->qResult = container.query(containerFilter, queryError).getQueryResult();
  // Check if error
  if (queryError) {
    std::cout << "Error! Could not load container contents." << std::endl
              << queryError << std::endl;
    exitProgram(30);
  }
}
...
void Notice::loadContainerContents(cbe::ContainerId containerId) {
  ...
  cbe::CloudBackend::QueryError queryError;
  // Call query function with filter
  this->qResult = myCloudBackend.query(containerId, containerFilter, queryError).getQueryResult();
  if (queryError) {
    std::cout << "Error! Could not load container contents." << std::endl
              << queryError << std::endl;
    exitProgram(31);
...
void Notice::loadContainerObjects(cbe::Container container) {
  ...
  cbe::CloudBackend::QueryError queryError;
  // Call query function with filter
  this->qResult = container.query(objectFilter, queryError).getQueryResult();
  if (queryError) {
    std::cout << "Error! Could not load container contents." << std::endl
              << queryError << std::endl;
    exitProgram(33);
...
cbe::Object Notice::createMyNoticeObject(cbe::Container inContainer) {
  ...
  myObject = *inContainer.upload(qualFile1Name, uploadError);
  if (uploadError){
    std::cout << "Error!\n"
              << "Could not upload file." << std::endl;
    exitProgram(31);
  }

  cbe::Object::UpdateKeyValuesError ukvError;
  myObject.updateKeyValues(keyValues, ukvError);
  if (ukvError)
  {
    std::cout << "Error!\n"
              << "Could not update object key/values." << std::endl;
    exitProgram(32);
  }
...
void Notice::sendMsg() {
  ...
  myCloudBackend = cbe::CloudBackend::logIn(username, password, tenant, client, logInError);
